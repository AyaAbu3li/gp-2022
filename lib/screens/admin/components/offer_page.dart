...
class Offer_Page extends StatefulWidget {
  final String text;
  const Offer_Page(this.text);
  @override
  State<Offer_Page> createState() => _OfferPageRequestState();
}
class _OfferPageRequestState extends State<Offer_Page> {
  ...
  void addOffer() async {
    try{
      var res = await http.patch(Uri.parse("http://"+ip+":3000/offers/"+widget.text),
          headers: <String, String>{
            'Context-Type': 'application/json;charSet=UTF-8',
          },
          body: <String, String>{
      ...
      showDialog(
        ...
        builder: (context) =>
            AlertDialog(
              title: Text('Done'),
              content: Text("The offer has been successfully added"),
              actions: [
                TextButton(
                  child: Text('OK',
                      style: TextStyle(
